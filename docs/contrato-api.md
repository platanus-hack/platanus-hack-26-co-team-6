...
|---|---|---|
| `Caso`, `ExtraccionClinica` | Neid — `/api/triage` | todos |
| `Sede` | Zaid — ETL + PostGIS | todos |
| `Candidato`, `DesgloseScore` | Zaid (ETA) + Neid (score) | Juan |
| `Handshake` | Sebas — `/api/dispatch` | Juan, Sebas |
...
### `POST /api/handshake/respond` — Sebas ⭐
