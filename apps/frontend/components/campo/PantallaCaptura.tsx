 ...
 * camino principal.
 *
 * El textarea no desaparece nunca. Es la regla del carril: si niego el permiso
 * del micrófono, o el navegador es un Safari sin Web Speech, la app sigue
...
import { useNivelVoz } from "@/lib/useNivelVoz";
...
export function PantallaCaptura({
  ...
  // El nivel de voz se escribe directo en el nodo del orbe, sin pasar por
  // React: son 60 medidas por segundo. Ver useNivelVoz.
  const orbeRef = useRef<HTMLDivElement>(null);
  useNivelVoz(escuchando, orbeRef);
  ...
  return (
    <section className="flex flex-col items-center">
      <OrbeVoz estado={estadoOrbe} refExterna={orbeRef} />
      ...
      <p className="mt-1 mb-4 text-sm font-semibold min-h-6" role="status">
        {analizando ? (
          <span className="text-[color:var(--color-alerta)]">Analizando el caso…</span>
        ) : escuchando ? (
          <span className="text-[color:var(--color-info)]">Escuchando… habla normal</span>
        ) : sinSenal ? (
      ...
      <button
        onClick={onMicrofono}
        disabled={analizando}
        className={`w-full min-h-16 rounded-2xl font-bold text-lg border transition-colors
                    disabled:opacity-40 ${
                      escuchando
      ...
      <textarea
        ...
        rows={5}
        className="mt-3 w-full p-4 rounded-2xl text-base leading-relaxed
      ...
      {!vozSoportada && (
        <p className="mt-3 w-full text-xs text-[color:var(--color-alerta)]">
          Este navegador no tiene dictado por voz (Safari e iOS no lo traen).
          Escribe el caso: el flujo es idéntico.
        </p>
      )}
