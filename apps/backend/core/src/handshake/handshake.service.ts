...
export class HandshakeService {
  ...
  async procesarRespuesta(cuerpo: RespondRequest): Promise<RespondResponse> {
    // Antes de mirar nada: aplicar los vencimientos pendientes. Sin esto, una
    // respuesta que llega pasado el plazo se procesaría como válida solo
    // porque nadie había consultado el estado todavía — el resultado
    // dependería de si alguien tenía una consola abierta, que es la clase de
    // no-determinismo que no se puede depurar en tarima.
    this.almacen.expirarVencidos();
