...
describe('LimiteTasaGuard', () => {
  ...
  it('el techo compartido topa aunque cada actor esté bajo el suyo', () => {
    // Sin este eje, 200 actores del mismo turno tumban el sistema para todos
    // sin que ninguno pase su propio límite.
    const guard = new LimiteTasaGuard();
    let rebotes = 0;

    for (let persona = 0; persona < 20; persona += 1) {
      const actor = { ...ACTOR, sub: `actor-${persona}` };
  ...
  it('un servicio no se estorba con el turno humano', () => {
    const guard = new LimiteTasaGuard();
    agotar(guard, 'POST', '/dispatch');

    const otra: ActorSesion = { sub: 'svc:voz', tipo: 'servicio', alcance: [] };
